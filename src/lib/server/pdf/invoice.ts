...
export function buildInvoicePdf(
  ...
  if (settings.companyRegistrationNo) {
    doc
      ...
      .fillColor(C.muted)
      .text(
        "Reg No: " + settings.companyRegistrationNo,
        M,
        companyBottomY + 4,
        { width: CW / 2 },
      );
  ...
  doc
    ...
    .fontSize(18)
    .fillColor(C.dark)
    .text(invoice.invoiceNumber, rightX, y + 14, {
      width: rightW,
      align: "right",
    });
  ...
  for (const [label, value, overdue] of infoItems) {
    ...
    doc
      ...
      .text(value + (overdue ? " — OVERDUE" : ""), rightX, labelEnd + 2, {
        width: rightW,
        align: "right",
      });
    rightY = doc.y + 10;
  ...
  doc.text("QTY", TX_QTY, y, {
    width: COL_QTY,
    align: "center",
    characterSpacing: 0.5,
  });
  doc.text("UNIT PRICE", TX_PRICE, y, {
    width: COL_PRICE,
    align: "right",
    characterSpacing: 0.5,
  });
  doc.text("TOTAL", TX_TOTAL, y, {
    width: COL_TOTAL,
    align: "right",
    characterSpacing: 0.5,
  });
  ...
  for (const line of invoice.lines) {
    ...
    doc
      ...
      .fontSize(10)
      .fillColor(C.body)
      .text(String(line.quantity), TX_QTY, rowY, {
        width: COL_QTY,
        align: "center",
      });
    doc
      ...
      .fontSize(10)
      .fillColor(C.body)
      .text(fmt(line.unitPrice), TX_PRICE, rowY, {
        width: COL_PRICE,
        align: "right",
      });
    doc
      ...
      .fontSize(10)
      .fillColor(C.body)
      .text(fmt(line.lineTotal), TX_TOTAL, rowY, {
        width: COL_TOTAL,
        align: "right",
      });
  ...
  doc
    ...
    .fontSize(10)
    .fillColor(C.muted)
    .text(fmt(invoice.subtotal), TX_TOTAL, y, {
      width: COL_TOTAL,
      align: "right",
    });
  y += 18;
  ...
  doc
    ...
    .fontSize(10)
    .fillColor(C.muted)
    .text(fmt(invoice.taxAmount), TX_TOTAL, y, {
      width: COL_TOTAL,
      align: "right",
    });
  y += 14;
  ...
  doc
    ...
    .fontSize(10)
    .fillColor(C.dark)
    .text(`Total (${invoice.currency})`, M, y, {
  ...
  doc
    ...
    .fontSize(13)
    .fillColor(C.dark)
    .text(fmt(invoice.total), TX_TOTAL, y, {
      width: COL_TOTAL,
      align: "right",
    });
  y = doc.y + 16;
  ...
  // ── AMOUNT PAID ───────────────────────────────────────────────────────────────
  // From `paidMinor`, which is worked out from the settlements against this
  // invoice's side on the shared owed account. The old `invoices.amount_paid`
  // column is deprecated and stays 0 for anything issued from now on (D-10), so
  // reading it here would have printed "nothing paid" on an invoice the screen
  // showed as settled — an export that contradicts what it was exported from.
  const paid = fromMinor(invoice.paidMinor);
  if (paid > 0) {
    // Green tinted box
    ...
    doc
      ...
      .fontSize(10)
      .fillColor(C.green)
      .text(`Amount Paid: ${invoice.currency} ${fmt(paid)}`, M + 12, y + 20);
    y += 52;
  }
