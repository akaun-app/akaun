...
/**
 * One income record. A thin wrapper over the records service, kept at its old
 * URL.
 *
...
const MISSING = "That income record no longer exists.";

/** The record at this id, but only when it really is income. */
function incomeAt(params: Partial<Record<string, string>>) {
  const id = Number(params.id);
  if (!Number.isInteger(id) || id <= 0) return null;
  const record = getRecord(db, id);
  return record?.kind === LedgerRecordKind.Income ? record : null;
}

export const GET: RequestHandler = async ({ locals, params }) => {
  if (!hasPermission(locals, "income", "view")) return forbidden();

  const record = incomeAt(params);
  if (!record) return notFound(MISSING);
  return Response.json(record);
};

export const PATCH: RequestHandler = async ({ locals, params, request }) => {
  if (!hasPermission(locals, "income", "change")) return forbidden();
...
export const DELETE: RequestHandler = async ({ locals, params }) => {
  if (!hasPermission(locals, "income", "delete")) return forbidden();
