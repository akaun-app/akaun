...
/**
 * What the Income list is told, live.
 *
 * One emitter carries every record write now, so this stream picks its own out
 * of it: income records, plus the two events that carry no record of their own.
 ...
 * server, which matters because seeing income does not mean being allowed to
 * see expenses (contracts/events.md).
 *
 * A payment changes an income record's paid state without anyone editing it,
 * which is why `settlement-changed` is here at all.
 */
